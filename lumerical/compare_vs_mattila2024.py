...
def add_franta_material(sim, n_fn, k_fn):
    ...
    # Lumerical v241 requires 2 or 4 cols; SiO2 k≈0 in visible, 2 cols suffice
...
"""
    ...
    # grating_orders shape: (wl, pol, m_orders, n_orders)
    # In 2D Y-normal mode, n_orders axis has size 1 (only n=0 exists)
...
def main():
    ...
    for i, s in enumerate(samples):
        ...
        # Interpolate onto Mattila wavelength grid for MAE — use 0th-order R0_l
        R0_interp = np.interp(wl_nm, lam_l, R0_l)
        mae_0th   = np.mean(np.abs(R0_interp - s["eta_r"]))

        print(f"  done in {dt_l:.1f}s  MAE(0th-order)={mae_0th:.4f}")
        ...
        if HAS_TORCWA and n_fn is not None:
            ...
            print(f"  done in {time.time()-t0:.1f}s  MAE={mae_tw:.4f}")

        # ── Plot ───────────────────────────────────────────────────────────────
        ax.plot(wl_nm, s["eta_r"], "k-",  lw=2.5, label="Mattila 2024 (measured)")
        ax.plot(lam_l, R0_l, "b-", lw=1.8,
                label=f"Lumerical RCWA 0th-order Rs\nMAE={mae_0th:.4f}")
        ax.plot(lam_l, Rs_l, "b:", lw=1.0, alpha=0.4,
                label="Lumerical RCWA total Rs (all orders)")
        if R_tw is not None:
            ax.plot(wl_nm, R_tw, "g--", lw=1.5,
                    label=f"torcwa 0th-order  MAE={mae_tw:.4f}")
    ...
    fig.suptitle("Lumerical RCWA vs Mattila 2024\n"
